...
export function burst(samples, totalRadius, startTheta = 0, startPhi = 0){
    ...
    for (let i = 0; i < samples; i++){
        ...
        points.push(new THREE.Vector3(x, y, z).multiplyScalar(totalRadius))
    }
...
export function arc(samples, radius, start, end){
    ...
    for (let i = 0; i <= samples; i++){
        let perc = i/samples;
        let point = new THREE.Vector3().lerpVectors ( start, end, perc ).normalize().multiplyScalar(radius);
        points.push(point);
    }
